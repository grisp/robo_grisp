...
copy_release_tar(TargetNode, Version, RelName) ->
    RelNameStr = atom_to_list(RelName),

    % io:format("Version ~p \n",[Version]),
    ReleaseTarName = RelNameStr ++ "-" ++ Version ,
    {ok, Cwd} = file:get_cwd(),
    LocalPath = filename:join([ Cwd,"_build", "grisp", "rel", RelNameStr, ReleaseTarName ++ ".tar.gz"]),
    % io:format(LocalPath ++ "\n"),
    {ok, Data} = file:read_file(LocalPath),
    RemotePath = filename:join([  RelNameStr, "releases", Version, RelNameStr ++ ".tar.gz"]),
    % generate remote directory
    ok = erpc:call(TargetNode, filelib, ensure_dir, [RemotePath], ?TIMEOUT),
    % write the tarball
